...
instanceArtifactLocation='/home/ec2-user/seerBot'
...
su - ec2-user -c 'mkdir -p /home/ec2-user/seerBot'
...
su - ec2-user -c 'cd /home/ec2-user/seerBot'

echo 'Make DIR for logs'
su - ec2-user -c 'mkdir /home/ec2-user/seerBot/logs'

echo 'Copy the BotAdmin artifact from' $bucket 'with key' $s3commonKeyPrefix
su - ec2-user -c 'aws s3api get-object --bucket biz-bot-artifact --key bkane/BotAdminApp/seerBot/seerlogics-shared-bot-1.0.0-SNAPSHOT.war /home/ec2-user/seerBot/seerlogics-shared-bot-1.0.0-SNAPSHOT.war'
...
su - ec2-user -c 'aws s3api get-object --bucket biz-bot-artifact --key bkane/BotAdminApp/seerBot/apache-tomcat-9.0.33.tar.gz apache-tomcat-9.0.33.tar.gz'
...
su - ec2-user -c 'aws s3api get-object --bucket biz-bot-artifact --key bkane/BotAdminApp/seerBot/server.xml /home/ec2-user/installs/tomcat/conf/server.xml'
