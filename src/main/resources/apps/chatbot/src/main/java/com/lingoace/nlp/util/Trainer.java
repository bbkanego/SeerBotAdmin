 ...
 * https://www.programcreek.com/java-api-examples/?code=jprante/elasticsearch-analysis-opennlp/elasticsearch-analysis-opennlp-master/src/test/java/org/xbib/elasticsearch/index/analysis/opennlp/Trainer.java
 */
public class Trainer {
    ...
    public void trainNameFinder(final String trainingData, String outModel, String modelType) throws Exception {
        InputStreamFactory inputStreamFactory = new InputStreamFactory() {
            @Override
            public InputStream createInputStream() throws IOException {
                return this.getClass().getClassLoader().getResourceAsStream(trainingData);
            }
        };
        ...
        TokenNameFinderModel model = NameFinderME.train("en", modelType, samples, trainingParameters, tokenNameFinderFactory);
        //NameFinderME.train("en", "person", samples, Collections.<String, Object>emptyMap(), 200, 5);
        samples.close();
        URL url = this.getClass().getClassLoader().getResource(outModel);
        File outFileFileObj = new File(url.toURI().getPath());
        FileOutputStream out = new FileOutputStream(outFileFileObj);
        model.serialize(out);
        out.close();
    }
}
